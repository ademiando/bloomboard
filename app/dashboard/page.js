...
/**
 * Final single-file Portfolio Dashboard (app/dashboard/page.js)
 *
 * - "use client"; is first line to ensure client-only behavior.
 * - Designed to be dropped into Next.js app router route: app/dashboard/page.js
 * - Core features included (best-effort):
 *   - Persistent assets & transactions (localStorage)
 *   - Crypto prices via CoinGecko (bulk)
 *   - Stock prices: AlphaVantage for .JK (if API key present), then Finnhub proxy, then Yahoo fallback
 *   - TradingView charts for assets (client-side only), fallback to CoinGecko charts for crypto
 *   - Non-liquid assets support with YoY growth projection
 *   - Eye toggle to hide numeric values (percent remains visible)
 *   - Share image generator (respects eye state) and CSV export/import with BOM (spreadsheet-friendly)
 *   - Cake allocation chart (interactive, spaced slices)
 *   - Portfolio growth chart (candles-like + multi-line simplified)
 *   - Transactions with undo (restore) and delete
 *   - UI behaviors: animated add button (+ -> x), hover interactions, simple transitions
 *
 * Notes:
 * - If you want AlphaVantage: put API key in localStorage 'alpha_vantage_key' or set window.ALPHA_VANTAGE_KEY
 * - Server proxies assumed: /api/finnhub/quote, /api/yahoo/quote (kept like original)
 * - This is a big single file; if commit fails, run the cleaning steps below (remove BOM, ensure use client on first line).
 */
...
const COINGECKO_COIN_CHART = (id, days = 90) =>
...
function fmtMoney(val, ccy = "USD") {
  ...
  if (ccy === "IDR") return new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(n);
  return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 }).format(n);
}
function isoDate(ms) { try { return new Date(ms).toISOString(); } catch { return ""; } }
function normalizeIdr(v) { const n = Number(v); if (!n || isNaN(n)) return null; if (n > 1000) return Math.round(n); return Math.round(n * 1000); }
...
/* Non-liquid asset price growth using YoY compounding */
...
function CakeAllocation({ data = [], size = 220, inner = 56, gap = 0.04, displayTotal = "", displayCcy = "USD", usdIdr = 16000 }) {
  ...
  const maxOuter = size/2 - 6, minOuter = inner + 8;
  const maxValue = Math.max(...data.map(d => Math.max(0, d.value || 0)), 1);
  const scaleOuter = (v) => { if (!v || v <= 0) return inner + 6; const frac = v / maxValue; return Math.round(minOuter + frac * (maxOuter - minOuter)); };
  ...
  const formatFor = (v) => displayCcy === "IDR" ? fmtMoney((v||0)*usdIdr,"IDR") : fmtMoney(v||0,"USD");

  const onMouseEnter = (i, ev, d) => {
    setHoverIndex(i);
    const rect = wrapRef.current?.getBoundingClientRect();
    const px = (ev.clientX - (rect?.left || 0)) + 10;
    const py = (ev.clientY - (rect?.top || 0)) - 10;
    setTooltip({ show:true, x:px, y:py, html:`${d.name} • ${formatFor(d.value)}` });
  };
  const onMouseMove = (e) => {
    const rect = wrapRef.current?.getBoundingClientRect();
    const px = (e.clientX - (rect?.left || 0)) + 10;
    setTooltip(t => ({ ...t, x: px }));
  };
  ...
  return (
    <div ref={wrapRef} style={{ width: size, height: size, position: "relative" }}>
        ...
        {data.map((d,i) => {
          ...
          return (
            <g key={i} transform={transform}>
              <path d={path} fill={colorForIndex(i)} stroke="#000" strokeWidth={isHover ? 1.6 : 0.6}
                onMouseEnter={(ev)=>onMouseEnter(i,ev,d)} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave}
      ...
      <div style={{
        ...
        padding: "8px 10px", background: "#111827", border: "1px solid rgba(255,255,255,0.06)", color:"#E5E7EB",
        borderRadius:8, fontSize:12, pointerEvents:"none", opacity: tooltip.show ? 1 : 0, transition: "opacity 140ms", zIndex:40
      }}>{tooltip.html}</div>
    </div>
  );
}

/* ===================== SMALL CANDLES + MULTI-LINE GROWTH (SIMPLIFIED) ===================== */
function CandlesWithLines({ seriesMap = {}, displayCcy = "USD", usdIdr = 16000, rangeKey = "all", onHover }) {
  ...
  const convVal = (v) => displayCcy === "IDR" ? v * usdIdr : v;
  const convAll = (seriesMap["all"] || []).map(p => ({ t: p.t, v: convVal(p.v) }));
  const convKeys = ["crypto","stock","nonliquid"];
  const convCats = {}; convKeys.forEach(k => convCats[k] = (seriesMap[k] || []).map(p => ({ t: p.t, v: convVal(p.v) })));
  ...
  const count = timeframeMap[rangeKey] || Math.min(200, convAll.length);

  // bucket convAll into `count` candles
  const buckets = Array.from({length: Math.max(4, count)}, () => []);
  ...
  const seriesToPoints = (catSeries) => {
    if (!catSeries || catSeries.length===0) return [];
    const pts = [];
    for (let i=0;i<candles.length;i++) {
      const midT = candles[i].t;
      let nearest = catSeries[0], bestD = Math.abs(catSeries[0].t - midT);
      for (let j=1;j<catSeries.length;j++){ const d = Math.abs(catSeries[j].t - midT); if (d < bestD){ bestD = d; nearest = catSeries[j]; } }
      const x = xOfCandle(i), y = yOf(nearest.v);
  ...
  const overlayPts = {};
  convKeys.forEach(k => overlayPts[k] = seriesToPoints(convCats[k]));
    ...
    <div className="w-full overflow-hidden rounded" style={{ background: "transparent" }}>
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        ...
        {candles.map((c,i) => {
          ...
          const bodyTop = Math.min(openY, closeY), bodyHeight = Math.max(1, Math.abs(closeY - openY));
          return (
            <g key={i}>
        ...
        {["crypto","stock","nonliquid"].map(k => {
          ...
          const stroke = k==="crypto" ? "#FF6B6B" : k==="stock" ? "#6BCB77" : "#FFD93D";
          const widthStroke = k==="stock"?1.8:1.4;
          return <g key={k}><path d={path} stroke={stroke} strokeWidth={widthStroke} fill="none" strokeLinecap="round" strokeLinejoin="round" opacity={0.95} /></g>;
        })}

        {[0,1,2,3,4].map(i => {
          const v = min + (i/4)*(range); const y = yOf(v);
...
/* ===================== TRADINGVIEW WIDGET (client-only) ===================== */
    ...
    function mount() {
      try {
        if (window.TradingView && window.TradingView.widget) {
          const ct = document.getElementById(idRef.current); if (ct) ct.innerHTML = "";
...
/* ===================== COINGECKO MINI CHART ===================== */
function CryptoMiniChart({ coinId = "bitcoin", displayCcy="USD", usdIdr=16000, days = 90 }) {
  ...
  const vs = data.map(d=>d.v), min = Math.min(...vs), max = Math.max(...vs), range = Math.max(1e-8, max-min);
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h}><rect x="0" y="0" width={w} height={h} fill="#0b1220" /><path d={path} stroke="#FF6B6B" strokeWidth={2} fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>
...
export default function PortfolioDashboard() {
  /* ---------- persistent state ---------- */
  const loadAssets = () => { try { if (!isBrowser) return []; const raw = JSON.parse(localStorage.getItem("pf_assets_v2")||"[]"); if (!Array.isArray(raw)) return []; return raw.map(ensureNumericAsset); } catch { return []; } };
  const [assets, setAssets] = useState(loadAssets);

  const loadTx = () => { try { if (!isBrowser) return []; const raw = JSON.parse(localStorage.getItem("pf_transactions_v2")||"[]"); if (!Array.isArray(raw)) return []; return raw; } catch { return []; } };
  const [transactions, setTransactions] = useState(loadTx);

  const loadRealized = () => { try { if (!isBrowser) return 0; return toNum(localStorage.getItem("pf_realized_v2") || 0); } catch { return 0; } };
  const [realizedUSD, setRealizedUSD] = useState(loadRealized);

  const loadDisplayCcy = () => { try { if (!isBrowser) return "USD"; return localStorage.getItem("pf_display_ccy_v2") || "USD"; } catch { return "USD"; } };
  ...
  useEffect(()=> {
    function onPointerDown(e) { if (suggestionsRef.current && !suggestionsRef.current.contains(e.target)) setSuggestions([]); }
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        try {
          ...
          const raw = payload.quotes || payload.result || payload.items || [];
          const list = (Array.isArray(raw) ? raw : []).slice(0,120).map(it => {
  ...
  useEffect(() => {
    ...
    async function fetchFx() {
      try { setFxLoading(true); const res = await fetch(COINGECKO_USD_IDR); if (!mounted || !res.ok) return; const j = await res.json(); const raw = j?.tether?.idr; const n = normalizeIdr(raw); if (n) setUsdIdr(prev => (!prev || Math.abs(prev - n) / n > 0.0005 ? n : prev)); }
      catch(e) {} finally { if (mounted) setFxLoading(false); }
    }
    fetchFx(); const id = setInterval(fetchFx, 60_000); return ()=>{ mounted = false; clearInterval(id); };
  }, []);
  ...
  useEffect(()=> {
    ...
    pollCg(); const id = setInterval(pollCg, 6000); return ()=>{ mounted=false; clearInterval(id); };
  }, [isInitialLoading]);

  // poll stocks: alphavantage -> finnhub -> yahoo (fallback)
  useEffect(()=> {
    let mounted = true;
    async function pollStocks() {
      try {
        ...
        if (symbols.length === 0) { if (isInitialLoading) setIsInitialLoading(false); return; }

        let alphaKey = (isBrowser && (window.ALPHA_VANTAGE_KEY || localStorage.getItem('alpha_vantage_key'))) || null;
        const map = {};

        // Alphavantage for .JK if key exists
        if (alphaKey) {
          const jkSymbols = symbols.filter(s => String(s||"").toUpperCase().endsWith(".JK"));
          for (const s of jkSymbols) {
            try {
              const tickerForApi = s; // often Alphavantage requires full ticker; adjust if needed
              const url = ALPHAVANTAGE_GLOBAL_QUOTE(tickerForApi, alphaKey);
  ...
  async function addManualAsset() {
    ...
    if (searchMode === "crypto") {
      newAsset = ensureNumericAsset({
        id: `manual:cg:${typed}:${Date.now()}`, type: "crypto", coingeckoId: typed.toLowerCase(), symbol: typed.toUpperCase(), name: typed, shares:0, avgPrice:0, investedUSD:0, lastPriceUSD:0, marketValueUSD:0
      });
    } else {
      newAsset = ensureNumericAsset({
        id: `manual:yh:${typed}:${Date.now()}`, type: "stock", symbol: typed.toUpperCase(), name: typed.toUpperCase(), shares:0, avgPrice:0, investedUSD:0, lastPriceUSD:0, marketValueUSD:0
      });
    }
  ...
  function addNonLiquidAsset() {
    const name = nlName.trim(); const qty = toNum(nlQty); const priceInput = toNum(nlPrice); const purchaseDateMs = nlPurchaseDate ? new Date(nlPurchaseDate).getTime() : Date.now(); const yoy = toNum(nlYoy);
    ...
    const asset = ensureNumericAsset({
      ...
      createdAt: Date.now(), purchaseDate: purchaseDateMs, nonLiquidYoy: yoy, description: nlDesc || ""
    });
    setAssets(prev => [...prev, asset]);
  ...
  function openTradeModal(assetId, mode) {
    ...
    const defaultPrice = asset.lastPriceUSD || asset.avgPrice || 0; setTradeModal({ open:true, mode, assetId, defaultPrice });
  }
  ...
  function performBuy(qty, pricePerUnit) {
    const id = tradeModal.assetId; if (!id) return; const q = toNum(qty), p = toNum(pricePerUnit); if (q<=0||p<=0){ alert("Qty & price must be > 0"); return; }
    const cost = q * p;
  ...
  function performSell(qty, pricePerUnit) {
    const id = tradeModal.assetId; if (!id) return; const q = toNum(qty), p = toNum(pricePerUnit); if (q<=0||p<=0){ alert("Qty & price must be > 0"); return; }
    const a = assets.find(x=>x.id===id); if (!a) return; const oldShares = toNum(a.shares||0); if (q > oldShares) { alert("Cannot sell more than you own"); return; }
    const avg = toNum(a.avgPrice||0); const proceeds = q * p, costOfSold = q * avg, realized = proceeds - costOfSold;
    ...
    setAssets(prev => {
      ...
      return prev.map(x => x.id === id ? ensureNumericAsset({ ...x, shares: oldShares - q, investedUSD: x.investedUSD - costOfSold, avgPrice: (oldShares - q) > 0 ? ((x.investedUSD - costOfSold)/(oldShares - q)) : 0, lastPriceUSD: p, marketValueUSD: (oldShares - q) * p }) : ensureNumericAsset(x));
    });
    setTransactions(prev => [tx, ...prev].slice(0,1000));
    closeTradeModal();
  }
  ...
  function deleteTransaction(txId) {
    ...
    if (tx.type === "buy") {
      setAssets(prev => prev.flatMap(a => {
        ...
        const oldShares = toNum(a.shares||0); const newShares = Math.max(0, oldShares - tx.qty); const newInvested = Math.max(0, toNum(a.investedUSD||0) - tx.cost);
      ...
      setAssets(prev => {
        ...
        if (found) {
          return prev.map(a => {
            if (a.id !== tx.assetId) return a;
            const newShares = toNum(a.shares||0) + tx.qty; const newInvested = toNum(a.investedUSD||0) + tx.costOfSold; const newAvg = newShares > 0 ? (newInvested / newShares) : 0;
            return ensureNumericAsset({ ...a, shares: newShares, investedUSD: newInvested, avgPrice: newAvg, marketValueUSD: newShares * (a.lastPriceUSD || newAvg) });
          });
        } else {
          const asset = ensureNumericAsset({ id: tx.assetId, type: tx.assetType||"stock", symbol: tx.symbol, name: tx.name||tx.symbol, shares: tx.qty, avgPrice: tx.costOfSold/(tx.qty||1), investedUSD: tx.costOfSold, lastPriceUSD: tx.pricePerUnit || (tx.costOfSold/(tx.qty||1)), marketValueUSD: tx.qty * (tx.pricePerUnit || (tx.costOfSold/(tx.qty||1))), createdAt: Date.now() });
          return [...prev, asset];
        }
      });
  ...
  function restoreTransaction(tx) {
    ...
    if (tx.type === "buy") {
      setAssets(prev => prev.map(a => {
        if (a.id !== tx.assetId) return a;
        const oldShares = toNum(a.shares||0), oldInvested = toNum(a.investedUSD||0); const newShares = oldShares + tx.qty, newInvested = oldInvested + tx.cost; const newAvg = newShares > 0 ? newInvested / newShares : 0;
      ...
      setAssets(prev => prev.map(a => {
        if (a.id !== tx.assetId) return a;
        const oldShares = toNum(a.shares||0); const newShares = Math.max(0, oldShares - tx.qty); const newInvested = Math.max(0, toNum(a.investedUSD||0) - tx.costOfSold); const newAvg = newShares > 0 ? newInvested / newShares : 0;
        return ensureNumericAsset({ ...a, shares: newShares, investedUSD: newInvested, avgPrice: newAvg, marketValueUSD: newShares * (tx.pricePerUnit || a.lastPriceUSD || newAvg) });
      }));
      setRealizedUSD(prev => prev + toNum(tx.realized||0));
    }
  ...
  /* ===================== CSV export/import (single combined, spreadsheet-friendly) ===================== */
  function csvQuote(v) {
  ...
  function exportAllCSV() {
    ...
    lines.push(`#ASSETS`); lines.push(assetsHeaders.join(","));
    assets.forEach(a => {
      const aa = ensureNumericAsset(a);
    ...
    lines.push("");
    lines.push(`#TRANSACTIONS`); lines.push(txHeaders.join(","));
    transactions.forEach(t => {
      const row = txHeaders.map(h => { const v = t[h]; if (h==="date") return csvQuote(isoDate(v)); if (typeof v === "number") return String(v); return csvQuote(v); }).join(",");
      lines.push(row);
    });
  ...
  function handleImportFile(file, { merge = true } = {}) {
    ...
    reader.onload = (e) => {
      ...
      let importedAssets = [], importedTx = [];

      if (idxAssets >= 0) {
        let headerLineIdx = -1;
        for (let i = idxAssets+1; i < lines.length; i++) { if (lines[i].trim() === "") continue; headerLineIdx = i; break; }
        if (headerLineIdx >= 0) {
          const headers = lines[headerLineIdx].split(",").map(h => h.replace(/^"|"$/g,"").trim());
          for (let i = headerLineIdx+1; i < lines.length; i++) {
            const l = lines[i]; if (!l || l.startsWith("#TRANSACTIONS") || l.startsWith("#META") || l.startsWith("#FILE") || l.startsWith("#EXPORT")) break;
            const values = []; let cur = "", insideQuote = false;
            for (let k=0;k<l.length;k++){ const ch = l[k]; if (ch === '"' && l[k+1] === '"') { cur += '"'; k++; continue; } if (ch === '"') { insideQuote = !insideQuote; continue; } if (ch === "," && !insideQuote) { values.push(cur); cur = ""; continue; } cur += ch; }
            values.push(cur);
      ...
      if (idxTx >= 0) {
        let headerLineIdx = -1;
        for (let i = idxTx+1; i < lines.length; i++) { if (lines[i].trim() === "") continue; headerLineIdx = i; break; }
        if (headerLineIdx >= 0) {
          const headers = lines[headerLineIdx].split(",").map(h => h.replace(/^"|"$/g,"").trim());
          for (let i = headerLineIdx+1; i < lines.length; i++) {
            const l = lines[i]; if (!l || l.startsWith("#META") || l.startsWith("#FILE") || l.startsWith("#EXPORT")) break;
            const values = []; let cur = "", insideQuote = false;
            for (let k=0;k<l.length;k++){ const ch = l[k]; if (ch === '"' && l[k+1] === '"') { cur += '"'; k++; continue; } if (ch === '"') { insideQuote = !insideQuote; continue; } if (ch === "," && !insideQuote) { values.push(cur); cur = ""; continue; } cur += ch; }
            values.push(cur);
            const obj = {}; headers.forEach((h, idx) => { obj[h] = values[idx] ?? ""; });
      ...
      if (importedAssets.length > 0) {
        if (merge) {
          const map = {}; assets.forEach(a=>map[a.symbol]=ensureNumericAsset(a)); importedAssets.forEach(i=>map[i.symbol]=ensureNumericAsset(i)); const merged = Object.values(map); setAssets(merged);
        } else setAssets(importedAssets);
      }
      if (importedTx.length > 0) {
        if (merge) setTransactions(prev => [...importedTx, ...prev].slice(0,1000));
        else setTransactions(importedTx.slice(0,1000));
      }
      alert("Import complete");
    };
    reader.readAsText(file);
  }
  function onImportClick(e) { const file = e.target.files && e.target.files[0]; if (!file) return; const replace = confirm("Replace existing portfolio & transactions? (OK = replace, Cancel = merge)"); handleImportFile(file, { merge: !replace }); e.target.value = ""; }

  /* ===================== SHARE IMAGE (respects eye state) ===================== */
  function generateShareImage() {
    try {
      const allocation = donutData.map((d,i) => ({ name: d.name, pct: (totals.market>0? (d.value/totals.market)*100 : 0).toFixed(2), value: valuesHidden ? null : d.value }));
      const payload = { totals: { invested: (valuesHidden? null : totals.invested), market: (valuesHidden? null : totals.market), pnl: (valuesHidden? null : totals.pnl), pnlPct: totals.pnlPct }, allocation, asOf: Date.now(), displayCcy };
      const canvas = document.createElement("canvas"); canvas.width = 800; canvas.height = 600; const ctx = canvas.getContext("2d");
      ctx.fillStyle = "#0b1220"; ctx.fillRect(0,0,canvas.width,canvas.height);
      ctx.fillStyle = "#E5E7EB"; ctx.font = "18px Inter, Arial, serif"; ctx.fillText("Portfolio Share", 28, 36);
      ctx.font = "12px monospace"; ctx.fillText(`AsOf: ${new Date(payload.asOf).toLocaleString()}`, 28, 56); ctx.fillText(`Ccy: ${payload.displayCcy}`, 28, 74);
      ctx.fillText("Allocation (pct)", 28, 100); let y = 120;
      payload.allocation.slice(0,8).forEach(a => { ctx.fillText(`${a.name}: ${a.pct}%${a.value ? " • " + (displayCcy==="IDR"? fmtMoney(a.value*usdIdr,"IDR"): fmtMoney(a.value,"USD")) : ""}`, 28, y); y += 20; });
      const url = canvas.toDataURL("image/png"); const a = document.createElement("a"); a.href = url; a.download = `portfolio_share_${Date.now()}.png`; document.body.appendChild(a); a.click(); a.remove();
    } catch (e) { alert("Share failed"); }
  }

  /* ===================== Asset chart open logic (TradingView mapping heuristic) ===================== */
  function mapSymbolForTradingView(asset) {
    if (!asset) return null;
    if (asset.type === "stock") {
      if (String(asset.symbol||"").toUpperCase().endsWith(".JK")) {
        const t = asset.symbol.replace(".JK",""); return `IDX:${t}`;
      }
      return asset.symbol;
    }
  ...
  async function openAssetChart(asset) { if (!asset) return; setAssetChartOpen({ open:true, asset }); }
  function closeAssetChart() { setAssetChartOpen({ open:false, asset:null }); }

  /* ===================== Growth series build (simplified) ===================== */
  function buildSeries(rows, txs, rangeKey) {
    const now = Date.now(); let earliest = now;
    ...
    let points = rangeKey==="1d"?48:rangeKey==="2d"?96:rangeKey==="1w"?56:rangeKey==="1m"?90:rangeKey==="1y"?180:200;
    const times = []; const step = Math.max(1, Math.floor((Date.now()-start)/points));
    ...
    for (const ts of times) {
      ...
      rows.forEach(r => {
        ...
        if (r.type === "nonliquid") priceAtT = computeNonLiquidLastPrice(r.avgPrice, r.purchaseDate||r.createdAt, r.nonLiquidYoy||0, ts);
        else {
          const assetTxs = transactions.filter(tx=>tx.assetId===r.id && tx.date <= ts);
          if (assetTxs.length > 0) priceAtT = assetTxs[assetTxs.length-1].pricePerUnit || priceAtT;
        }
        const qty = (() => {
          const txsForAsset = transactions.filter(tx => tx.assetId === r.id && tx.date <= ts);
          let s = 0; txsForAsset.forEach(tx=>{ if (tx.type==="buy") s += toNum(tx.qty); if (tx.type==="sell") s -= toNum(tx.qty); });
          if (txsForAsset.length === 0) s = toNum(r.shares || 0); return s;
        })();
        const v = priceAtT * qty; total += v; if (r.type==="crypto") totalCrypto += v; else if (r.type==="stock") totalStock += v; else totalNon += v;
      });
      series.all.push({ t: ts, v: total }); series.crypto.push({ t: ts, v: totalCrypto }); series.stock.push({ t: ts, v: totalStock }); series.nonliquid.push({ t: ts, v: totalNon });
    }
    return series;
  }
  ...
  return (
    <div className="min-h-screen bg-black text-gray-200 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-gray-400 flex items-center gap-2 mt-1">
              {isInitialLoading && assets.length>0 ? (<><svg className="animate-spin h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>Loading portfolio data...</span></>) : ( lastTick && (<><span>Updated: {new Date(lastTick).toLocaleString()}</span><span>•</span><span className="flex items-center gap-1">USD/IDR ≈ {fxLoading ? (<svg className="animate-spin h-3 w-3 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>) : usdIdr?.toLocaleString()}</span></>) )}
            </div>
          </div>
          ...
          <div className="mt-6 bg-transparent p-3 rounded">
            ...
            {searchMode !== "nonliquid" ? (
              <div className="flex gap-3 flex-col sm:flex-row items-start">
                ...
                <select value={initPriceCcy} onChange={(e)=>setInitPriceCcy(e.target.value)} className="rounded-md bg-gray-900 px-2 py-2 text-sm border border-gray-800"><option value="USD">USD</option><option value="IDR">IDR</option></select>
                <div className="flex items-center gap-2"><button onClick={()=> selectedSuggestion ? addAssetFromSuggestion(selectedSuggestion) : addManualAsset()} className="bg-emerald-500 hover:bg-emerald-400 text-black px-4 py-2 rounded font-semibold">Add</button><button onClick={addAssetWithInitial} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded font-semibold">Add + Position</button><button onClick={()=>setOpenAdd(false)} className="bg-gray-800 px-3 py-2 rounded">Close</button></div>
              </div>
            ) : (
        ...
        {/* GROWTH CHART */}
        <div className="mt-6 bg-transparent p-3 rounded">
          <div className="flex items-center justify-between mb-3"><div className="text-sm text-gray-400">Portfolio Growth</div><div className="flex items-center gap-2">{["1d","2d","1w","1m","1y","all"].map(k=> (<button key={k} onClick={()=>setChartRange(k)} className={`text-xs px-2 py-1 rounded ${chartRange===k?'bg-gray-800':'bg-transparent'}`}>{k}</button>))}</div></div>
          <div><CandlesWithLines seriesMap={growthSeries} displayCcy={displayCcy} usdIdr={usdIdr} rangeKey={chartRange} /></div>
        </div>

        {/* ASSET TABLE */}
        <div className="mt-6 overflow-x-auto"><div style={{ minWidth:940 }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div><button onClick={exportAllCSV} className="bg-white text-black px-3 py-1 rounded">Export</button></div>
              <label className="bg-white text-black px-3 py-1 rounded cursor-pointer">Import<input type="file" accept=".csv,text/csv" onChange={onImportClick} className="hidden" /></label>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {sortedRows.length === 0 ? (<tr><td colSpan={7} className="py-8 text-center text-gray-500">No assets — add one with the + button</td></tr>) : sortedRows.map(r => (
                <tr key={r.id} className="border-b border-gray-900 hover:bg-gray-950">
                  <td className="px-3 py-3"><div className="font-semibold text-gray-100">{r.symbol}</div><div className="text-xs text-gray-400">{r.name}{r.description ? ` • ${r.description}` : ""}</div></td>
                  <td className="px-3 py-3 text-right">{Number(r.shares||0).toLocaleString(undefined, { maximumFractionDigits: 8 })}</td>
                  <td className="px-3 py-3 text-right tabular-nums"><div className="font-medium">{ valuesHidden ? "*****" : (displayCcy==="IDR"? fmtMoney(r.investedUSD*usdIdr,"IDR") : fmtMoney(r.investedUSD,"USD")) }</div><div className="text-xs text-gray-400">{ displayCcy==="IDR" ? fmtMoney(r.avgPrice*usdIdr,"IDR") : fmtMoney(r.avgPrice,"USD") }</div></td>
                  <td className="px-3 py-3 text-right tabular-nums"><div className="font-medium">{ valuesHidden ? "*****" : (displayCcy==="IDR"? fmtMoney(r.marketValueUSD*usdIdr,"IDR") : fmtMoney(r.marketValueUSD,"USD")) }</div><div className="text-xs text-gray-400">{ displayCcy==="IDR" ? fmtMoney(r.lastPriceUSD*usdIdr,"IDR") : fmtMoney(r.lastPriceUSD,"USD") }</div></td>
                  <td className="px-3 py-3 text-right"><div className={`font-semibold ${r.pnlUSD >= 0 ? "text-emerald-400" : "text-red-400"}`}>{ valuesHidden ? "*****" : (displayCcy==="IDR"? fmtMoney(r.pnlUSD*usdIdr,"IDR") : fmtMoney(r.pnlUSD,"USD")) }</div><div className={`text-xs ${r.pnlUSD >= 0 ? "text-emerald-400" : "text-red-400"}`}>{isFinite(r.pnlPct) ? `${r.pnlPct.toFixed(2)}%` : "0.00%"}</div></td>
                  <td className="px-3 py-3 text-right"><div className="flex items-center justify-end gap-2"><button onClick={()=>openTradeModal(r.id,"buy")} className="bg-emerald-500 px-2 py-1 rounded text-xs font-semibold text-black">Buy</button><button onClick={()=>openTradeModal(r.id,"sell")} className="bg-yellow-600 px-2 py-1 rounded text-xs">Sell</button><button onClick={()=> removeAssetConfirm(r.id)} className="bg-red-600 px-2 py-1 rounded text-xs font-semibold text-black">Del</button><button onClick={()=>openAssetChart(r)} className="bg-gray-700 px-2 py-1 rounded text-xs">Chart</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {filteredRows.length > 0 && (
          <div className="mt-6 flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {donutData.map((d,i)=> { const pct = totals.market > 0 ? (d.value / totals.market) * 100 : 0;
                return (<div key={d.name} className="flex items-center gap-3"><div style={{ width:12, height:12, background: colorForIndex(i) }} className="rounded-sm" /><div><div className="font-semibold text-gray-100">{d.name}</div><div className="text-xs text-gray-400">{ displayCcy==="IDR" ? (valuesHidden ? "*****" : fmtMoney(d.value*usdIdr,"IDR")) : (valuesHidden ? "*****" : fmtMoney(d.value,"USD")) } • {pct.toFixed(1)}%</div></div></div>); })}
            </div>
          </div>
        )}
        ...
        {assetChartOpen.open && assetChartOpen.asset && (
          <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[110]">
            <div className="bg-gray-900 p-4 rounded-lg w-full max-w-4xl border border-gray-800">
              <div className="flex justify-between items-start"><div><h3 className="text-lg font-semibold">{assetChartOpen.asset.symbol} · {assetChartOpen.asset.name}</h3><div className="text-xs text-gray-400">{assetChartOpen.asset.description || ""}</div></div><div className="flex items-center gap-2"><button onClick={()=>generateShareImage()} className="bg-white text-black px-3 py-1 rounded">Share</button><button onClick={()=>closeAssetChart()} className="text-gray-400 hover:text-white">Close</button></div></div>
              <div className="mt-4">{ assetChartOpen.asset.type === "crypto" ? <AssetChartForCrypto asset={assetChartOpen.asset} displayCcy={displayCcy} usdIdr={usdIdr} /> : <TradingViewWidget symbol={mapSymbolForTradingView(assetChartOpen.asset)} height={420} /> }</div>
            </div>
          </div>
        )}
      ...
      {/* Trade modal */}
      {tradeModal.open && <TradeModal mode={tradeModal.mode} asset={assets.find(a=>a.id===tradeModal.assetId)} defaultPrice={tradeModal.defaultPrice} onClose={()=>setTradeModal({ open:false })} onBuy={performBuy} onSell={performSell} usdIdr={usdIdr} />}

    </div>
  );

  function removeAssetConfirm(id) { const a = assets.find(x=>x.id===id); if (!a) return; if (!confirm(`Delete ${a.symbol} (${a.name||""}) from portfolio?`)) return; setAssets(prev=>prev.filter(x=>x.id!==id)); }
}

/* ===================== AssetChartForCrypto component ===================== */
function AssetChartForCrypto({ asset, displayCcy="USD", usdIdr=16000 }) {
  ...
  useEffect(()=> {
    if (!asset) return;
    const sym = (asset.symbol||"").replace(/^@/,"").replace(/-USD$/i,"").toUpperCase();
    const candidates = [`BINANCE:${sym}USDT`, `COINBASE:${sym}USD`, `${sym}USD`, `CRYPTO:${sym}USD`];
    setTvSymbol(candidates[0]);
  }, [asset]);
  if (!asset) return null;
  return (
    <div>
      {preferTv ? (<div><TradingViewWidget symbol={tvSymbol || asset.symbol} height={420} /><div className="mt-2 flex gap-2"><button onClick={()=>setPreferTv(false)} className="px-2 py-1 rounded bg-gray-800">Use CoinGecko chart</button></div></div>) : (<div><CryptoMiniChart coinId={(asset.coingeckoId || asset.symbol || "").toLowerCase()} displayCcy={displayCcy} usdIdr={usdIdr} days={90} /><div className="mt-2 flex gap-2"><button onClick={()=>setPreferTv(true)} className="px-2 py-1 rounded bg-gray-800">Try TradingView</button></div></div>)}
    </div>
  );
}
...
function TradeModal({ mode, asset, defaultPrice, onClose, onBuy, onSell, usdIdr }) {
  ...
  function handleSubmit(e) { e.preventDefault(); const q = toNum(qty), p = priceUSD; if (q<=0||p<=0){ alert("Qty & price must be > 0"); return; } if (mode === 'buy') onBuy(q,p); if (mode === 'sell') onSell(q,p); }
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[120]">
      <div className="bg-gray-900 p-6 rounded-lg w-full max-w-md border border-gray-800">
        <div className="flex justify-between items-start"><div><h2 className="text-xl font-semibold capitalize">{mode} {asset.symbol}</h2><p className="text-sm text-gray-400">{asset.name}</p></div><button onClick={()=>onClose && onClose()} className="text-gray-500 hover:text-white">×</button></div>
