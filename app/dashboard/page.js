...
/**
 * app/dashboard/page.js
 * Single-file Portfolio Dashboard â€” updated fixes:
 * - Fix sort/filter dropdown scrolling (not clipped) â€” uses overflow-y: visible on table wrapper
...
function CakeAllocation({ data = [], size = 200, inner = 48, gap = 0.02, displayTotal, displayCcy = "USD", usdIdr = 16000 }) {
  ...
  const onSliceEnter = (i, event, d) => {
    ...
    setTooltip({ show: true, x: px, y: py, html: `${d.name} â€¢ ${formatForDisplayCcy(d.value)}` });
  };
...
export default function PortfolioDashboard() {
  ...
  async function addManualAsset() {
    const typed = query.split("â€”")[0].trim();
  ...
  function addNonLiquidAsset() {
    ...
    const asset = ensureNumericAsset({
      id,
      type: "nonliquid",
      symbol: (name.length > 12 ? name.slice(0, 12) + "â€¦" : name).toUpperCase(),
      name,
      shares: qty,
      avgPrice: priceUSD,
        ...
        <div className="mt-2 text-xs text-gray-400 flex items-center gap-2">
          {isInitialLoading && assets.length > 0 ? (
            ...
            <>
              ...
              <span>â€¢</span>
              <span className="flex items-center gap-1">USD/IDR â‰ˆ {fxLoading ? (
                ...
                <div className="relative w-full sm:max-w-lg">
                  ...
                  {suggestions.length > 0 && (
                    <div ref={suggestionsRef} className="absolute z-50 mt-1 w-full bg-gray-950 border border-gray-800 rounded max-h-56 overflow-auto">
                      {suggestions.map((s, i) => (
                        <button key={i} onClick={() => { setSelectedSuggestion(s); setQuery(`${s.symbol} â€” ${s.display}`); setSuggestions([]); }} className="w-full px-3 py-2 text-left hover:bg-gray-900 flex justify-between">
                          <div>
                            <div className="font-medium text-gray-100">{s.symbol} â€¢ {s.display}</div>
                            <div className="text-xs text-gray-500">{s.source === "coingecko" ? "Crypto" : `Security â€¢ ${s.exchange || ''}`}</div>
                          </div>
                        </button>
                      ))}
                    </div>
        ...
        */}
        <div className="mt-6" style={{ overflowX: 'auto', overflowY: 'visible' }}>
            ...
            <div className="flex items-center gap-2 relative">
              ...
              {sortMenuOpen && (
                <div ref={sortMenuRef} className="absolute right-0 mt-2 bg-gray-800 border border-gray-700 rounded shadow-lg overflow-hidden w-56 z-40 menu-scroll">
                  <button onClick={() => { setSortBy("market_desc"); setSortMenuOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-gray-700">Value (high â†’ low)</button>
                  <button onClick={() => { setSortBy("invested_desc"); setSortMenuOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-gray-700">Invested (high â†’ low)</button>
                  <button onClick={() => { setSortBy("pnl_desc"); setSortMenuOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-gray-700">P&L (high â†’ low)</button>
                  <button onClick={() => { setSortBy("symbol_asc"); setSortMenuOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-gray-700">A â†’ Z</button>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {sortedRows.length === 0 ? (
                <tr><td colSpan={6} className="py-8 text-center text-gray-500">No assets â€” add one with the + button</td></tr>
              ) : sortedRows.map((r) => (
                    ...
                    <div>
                      ...
                      {d.name === "Other" ? (
                        <div className="text-xs text-gray-400">
                          {d.symbols.join(', ')} <br/>
                          {displayCcy === "IDR" ? fmtMoney(d.value * usdIdr, "IDR") : fmtMoney(d.value, "USD")} â€¢ {pct.toFixed(1)}%
                        </div>
                      ) : (
                        <div className="text-xs text-gray-400">
                          {displayCcy === "IDR" ? fmtMoney(d.value * usdIdr, "IDR") : fmtMoney(d.value, "USD")} â€¢ {pct.toFixed(1)}%
