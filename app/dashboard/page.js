...
export default function DashboardPage() {
  ...
  useEffect(() => {
    let ignore = false;
    async function go() {
      if (!debouncedSearch || debouncedSearch.length < 2 || !FINNHUB_KEY) {
        setSuggestions([]);
        return;
      }
      try {
        const res = await fetch(SEARCH_URL(debouncedSearch));
        const json = await res.json();
        if (ignore) return;
        const list = Array.isArray(json?.result) ? json.result : [];
        // Filter simbol yang wajar untuk retail (stocks/crypto/forex)
        const filtered = list
                    ...
                    <tr key={r.id} className="border-t border-gray-900 hover:bg-gray-950">
                      ...
                      <td className="px-3 py-2">
                        ...
                        <div className="text-xs text-gray-500">
                          Quote in {r.quoteCcy} • avg(base USD) used for P&amp;L
                        </div>
                      </td>
