...
/**
 * Bloomboard Portfolio - Final, Complete, and Fully Functional High-Fidelity Clone
 *
 * This definitive version meticulously recreates the Stockbit UI from the provided screenshots
 * and integrates ALL original functionalities from the user's complex source file.
 * Every button, modal, and feature is now active and fully functional, with no placeholders.
 *
 * KEY FUNCTIONALITIES:
 * - Full-Featured "Add Asset" Modal: With multi-category tabs (Stocks ID/US, Crypto, Non-Liquid) and flexible auto-calculating inputs.
 * - Active CSV Import/Export: Fully wired to the original functions.
 * - Accurate Performance Area Chart: As seen in the reference image.
 * - Interactive Donut Chart: Correctly placed and styled within the Performance page.
 * - Functional Buy/Sell Trade Modal: With flexible inputs.
 * - All Systems Integrated: Balance, assets, and transactions are fully interconnected.
 * - Real-time USD/IDR FX Rate in Header.
 * - Fixed Indonesian Stock Search.
 * - Dynamic USD/IDR display toggle that affects the entire app.
 * - High-precision decimals for USD and percentages.
 */
...
function fmt(val, type="IDR", digits=0) { if (type === "USD") return new Intl.NumberFormat("en-US", { style: "decimal", minimumFractionDigits: 2, maximumFractionDigits: 4 }).format(val || 0); return new Intl.NumberFormat("id-ID", { style: "decimal", minimumFractionDigits: digits, maximumFractionDigits: digits }).format(val || 0); }
...
            let priceMap = {};
            if (stockSymbols.length > 0) { for (const symbol of stockSymbols) { try { const res = await fetch(FINNHUB_QUOTE(symbol)); const data = await res.json(); const price = toNum(data?.c); if (price > 0) { const isIdr = symbol.toUpperCase().endsWith('.JK'); priceMap[symbol] = { [isIdr ? 'idr' : 'usd']: price }; } } catch (e) {} } }
