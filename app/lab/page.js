...
// Formatting:
// - IDR: no decimals, prefix "IDR "
// - USD: two decimals, prefix "USD "
function fmtCurrency(val, ccy = "IDR") {
    const n = Number(val || 0);
    if (ccy === "USD") {
        return `USD ${n.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
    }
    // IDR: round to integer, no decimals
    return `IDR ${Math.round(n).toLocaleString('id-ID')}`;
}

// Format shares: if <1 show up to 6 decimals, otherwise no decimals
function fmtShares(val) {
    const n = Number(val || 0);
    if (Math.abs(n) < 1 && n !== 0) return n.toFixed(6).replace(/(?:\.0+|(\.\d+?)0+)$/, "$1");
...
export default function PortfolioDashboard() {
    ...
    // Poll prices: stocks (finnhub) + crypto (coingecko)
    useEffect(() => {
        const pollPrices = async () => {
            ...
            if (cryptoIds.length > 0) {
                try {
                    ...
                    for (const id of cryptoIds) {
                        if (j[id]) {
                            newPrices[id] = { usd: toNum(j[id].usd), idr: toNum(j[id].idr) };
                        }
                    }
                } catch (e) {}
            }
            ...
            setAssets(prev => prev.map(a => {
                if (a.type === 'stock' && newPrices[a.symbol]) {
                    ...
                    const priceData = newPrices[a.symbol];
                ...
                if (a.type === 'crypto' && a.coingeckoId && newPrices[a.coingeckoId]) {
                    const priceData = newPrices[a.coingeckoId];
                    const lastPriceUSD = priceData.usd || a.lastPriceUSD;
                    return { ...a, lastPriceUSD };
                }
                return a;
            }));
    ...
    // suggestions search (yahoo / coingecko)
    ...
    // Delete/liquidate asset at market price
    ...
    const exportCSV = () => alert("CSV Export preserved (not implemented here).");
    const importCSV = (e) => { alert("CSV Import preserved (not implemented here)."); e.target.value = ''; };

    // Helper: clear all
    const clearAllData = () => {
        if (!confirm("Erase all portfolio data? This cannot be undone.")) return;
        ...
        <div className="bg-black text-gray-300 min-h-screen font-sans">
            <div className="max-w-4xl mx-auto">
                <header className="p-4 flex justify-between items-center sticky top-0 bg-black z-10">
                    ...
                    {/* Switch + rate (switch shows active currency, rate to the right) */}
                    <div className="flex items-center gap-3">
                        <div
                            role="switch"
                            aria-checked={displayCcy === 'USD'}
                            onClick={() => setDisplayCcy(prev => prev === 'IDR' ? 'USD' : 'IDR')}
                            className={`relative w-16 h-8 rounded-full p-1 cursor-pointer ${displayCcy === 'USD' ? 'bg-emerald-600' : 'bg-gray-700'}`}
                        >
                            <div className={`absolute top-1 left-1 w-6 h-6 rounded-full bg-white transition-transform ${displayCcy === 'USD' ? 'translate-x-8' : 'translate-x-0'}`}></div>
                            <div className="absolute inset-0 flex items-center justify-center text-xs font-semibold text-black">{displayCcy}</div>
                        </div>
                ...
                <main>
                    <div className="border-b border-gray-800 px-4">
                        <nav className="flex space-x-6"><button className="py-2 px-1 border-b-2 font-semibold text-sm border-emerald-400 text-white">PORTFOLIO</button></nav>
                    </div>
                    ...
                    <section className="p-4">
                        <div className="grid grid-cols-3 gap-px bg-[#0a0a0a] p-px">
                            <div className="bg-black p-2">
                                ...
                                <p className="font-semibold text-sm text-white">{fmtCurrency(displayCcy === 'IDR' ? tradingBalance : Math.round(tradingBalance / usdIdr), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p>
                            </div>

                            <div className="bg-black p-2">
                                <p className="text-xs text-gray-500">Invested</p>
                                <p className="font-semibold text-sm text-white">{fmtCurrency(totals.invested * usdIdr * (displayCcy === 'IDR' ? 1 : 1/usdIdr), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p>
                            </div>

                            <div className="bg-black p-2">
                                <p className="text-xs text-gray-500">Total Equity</p>
                                <p className="font-semibold text-sm text-white">{fmtCurrency(totalEquity * (displayCcy === 'IDR' ? 1 : 1/usdIdr), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p>
                            </div>

                            {/* Combined Gain P&L tile */}
                            <div className="bg-black p-2 col-span-2">
                                ...
                                <p className={`font-semibold text-sm ${totals.pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                    {totals.pnl >= 0 ? '+' : ''}{fmtCurrency(totals.pnl * (displayCcy === 'IDR' ? usdIdr : 1), displayCcy === 'IDR' ? 'IDR' : 'USD')} ({Math.round(totals.pnlPct||0)}%)
                                </p>
                            </div>

                            <div className="bg-black p-2">
                                <p className="text-xs text-gray-500">Realized P&L</p>
                                <p className={`font-semibold text-sm ${realizedUSD >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{realizedUSD >= 0 ? '+' : ''}{fmtCurrency(realizedUSD * (displayCcy === 'IDR' ? usdIdr : 1), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p>
                            </div>
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="text-left text-gray-500 text-xs">
                                <tr>
                                    <th className="p-3 pt-4">Code<br/>Shares</th>
                            ...
                            <tbody>
                                {rows.map(r => {
                                    const factor = (displayCcy === 'IDR') ? usdIdr : 1;
                                    const currency = displayCcy === 'IDR' ? 'IDR' : 'USD';
                                    return (
                                        <tr key={r.id} className="border-t border-gray-800 hover:bg-gray-900/50 cursor-pointer" onClick={() => setTradeModal({ open: true, asset: r })}>
                                            <td className="p-3">
                                                ...
                                                <div className="text-gray-400">{fmtShares(r.shares)} Shares</div>
                                            </td>

                                            <td className="p-3 text-right tabular-nums">
                                                <div className="font-semibold text-white">{fmtCurrency(r.investedUSD * factor, currency)}</div>
                                                <div className="text-gray-400">Avg: {fmtCurrency(r.avgPrice * factor, currency)}</div>
                                            </td>

                                            <td className="p-3 text-right tabular-nums">
                                                <div className="font-semibold text-white">{fmtCurrency(r.marketValueUSD * factor, currency)}</div>
                                                <div className="text-gray-400">{fmtCurrency(r.lastPriceUSD * factor, currency)}</div>
                                            </td>

                                            <td className="p-3 text-right tabular-nums">
                                                <div className={`font-semibold ${r.pnlUSD >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{r.pnlUSD >= 0 ? '+' : ''}{fmtCurrency(r.pnlUSD * factor, currency)}</div>
                ...
                {/* Trade modal (buy/sell + trash icon) */}
                ...
                {activeTab === 'portfolio' ? (
                    <div className="p-4 space-y-8">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-white mb-1">{fmtCurrency(totalEquity * (displayCcy === 'IDR' ? 1 : 1/usdIdr), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p>
                            <p className={`font-semibold text-sm ${totals.pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{totals.pnl >= 0 ? '+' : ''}{fmtCurrency(totals.pnl * (displayCcy === 'IDR' ? usdIdr : 1), displayCcy === 'IDR' ? 'IDR' : 'USD')} ({Math.round(totals.pnlPct||0)}%) All Time</p>
                        </div>

                        <div>
                            <div className="mt-4">
                                <AreaChart equityData={[{t: Date.now() - 365*24*36e5, v: totalEquity*0.8}, {t: Date.now(), v: totalEquity}]} />
                            </div>
    ...
    return (
        <div className="p-4 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#181818] p-3 rounded-lg"><p className="text-sm text-gray-400">Max Profit</p><p className="text-lg font-semibold text-emerald-400">+{fmtCurrency(getVal(stats.maxProfit), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p></div>
                <div className="bg-[#181818] p-3 rounded-lg"><p className="text-sm text-gray-400">Max Loss</p><p className="text-lg font-semibold text-red-400">{fmtCurrency(getVal(stats.maxLoss), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p></div>
                <div className="bg-[#181818] p-3 rounded-lg"><p className="text-sm text-gray-400">Avg. Profit</p><p className="text-lg font-semibold text-emerald-400">+{fmtCurrency(getVal(stats.avgProfit), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p></div>
                <div className="bg-[#181818] p-3 rounded-lg"><p className="text-sm text-gray-400">Avg. Loss</p><p className="text-lg font-semibold text-red-400">{fmtCurrency(getVal(stats.avgLoss), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p></div>
            </div>

            <div><p className="text-sm text-gray-400">Total Realized Gain</p><p className="text-2xl font-bold text-emerald-400">+{fmtCurrency(getVal(stats.totalRealizedGain), displayCcy === 'IDR' ? 'IDR' : 'USD')}</p></div>
        </div>
    );
};
                    ...
                    <tbody>
                        {transactions.map(tx => (
                            <tr key={tx.id} className="border-t border-gray-800">
                                ...
                                <td className="p-3 text-right">{fmtShares(tx.qty)}</td>
                                <td className="p-3 text-right text-gray-400">{fmtCurrency((tx.pricePerUnit || tx.price || 0) * factor, currency)}</td>
                                <td className="p-3 text-right">{fmtCurrency((tx.cost || tx.proceeds || 0) * factor, currency)}</td>
                            </tr>
                        ))}
...
const AddAssetForm = ({ searchMode, setSearchMode, query, setQuery, suggestions, setSelectedSuggestion, setSuggestions, selectedSuggestion, addAssetWithInitial, addNonLiquidAsset, nlName, setNlName, nlQty, setNlQty, nlPrice, setNlPrice, nlPriceCcy, setNlPriceCcy, nlPurchaseDate, setNlPurchaseDate, nlYoy, setNlYoy, nlDesc, setNlDesc, usdIdr }) => {
    ...
    const handleInputChange = (field, value) => {
        if (field === 'shares') {
            ...
            if (numPrice > 0 && numShares > 0) setTotal(`${Math.round(numShares * numPrice)} ${ccy}`);
            else setTotal('');
        } else if (field === 'price') {
            setPrice(value);
            ...
            const numShares = toNum(shares);
            if (numShares > 0 && numTotal > 0) setPrice(String(Math.round(numTotal / numShares)));
        }
    };
...
const TradeModal = ({ isOpen, onClose, asset, onBuy, onSell, onDelete, usdIdr, displayCcy }) => {
    ...
    useEffect(() => {
        if (asset) {
            ...
            setPrice(String(isFinite(priceVal) ? (displayCcy === 'USD' ? priceVal.toFixed(2) : Math.round(priceVal)) : ''));
            setShares('');
            setTotal('');
    ...
    const handleInputChange = (field, value) => {
        if (field === 'shares') {
            ...
            if (nPrice > 0 && nShares > 0) setTotal(fmtCurrency(nPrice * nShares, ccy));
            else setTotal('');
        } else if (field === 'price') {
            setPrice(value);
    ...
    const isIdrAsset = asset.symbol && asset.symbol.endsWith('.JK');
...
const AreaChart = ({ equityData }) => {
    ...
    return (<svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}><defs><linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#22c55e" stopOpacity={0.3} /><stop offset="100%" stopColor="#22c55e" stopOpacity={0} /></linearGradient></defs><path d={areaPath} fill="url(#areaGradient)" /><path d={path} fill="none" stroke="#22c55e" strokeWidth="2" />{yAxisLabels.map(v => (<g key={v}><line x1={padding.left} x2={width-padding.right} y1={yScale(v)} y2={yScale(v)} stroke="rgba(255,255,255,0.05)" strokeDasharray="2,2" /><text x={width-padding.right+5} y={yScale(v)+4} fontSize="11" fill="#6B7280">{fmtLabel(v)}</text></g>))}</svg>);
};
...
const AllocationDonut = ({ data, displayCcy, usdIdr }) => {
    ...
    if (total === 0) return <div className="text-center text-gray-500 py-8">No assets to display</div>;
    ...
    const handleOver = (i,d)=>{ setHover(i); const pct = Math.round(d.value/total*100); const val = Math.round(d.value*(displayCcy==='IDR'?usdIdr:1)); setTooltip(t=>({...t,show:true,content:`${d.name}: ${displayCcy==='IDR'?'IDR':'USD'} ${new Intl.NumberFormat(displayCcy==='IDR'?'id-ID':'en-US').format(val)} (${pct}%)`})); };
        ...
        <div className="flex flex-col md:flex-row items-center gap-6" ref={ref} onMouseMove={handleMove}>
            <div className="relative" style={{width:size,height:size}}>
                ...
                <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}><g transform={`translate(${size/2}, ${size/2})`}>
                    {data.map((d,i)=>{ const angle = (d.value/total)*2*Math.PI; const start=cum; const end=cum+angle; cum=end; const isH= hover===i; const rOut = isH?outerRadius+6:outerRadius; const arc=(r,sa,ea)=>{ const large = ea-sa <= Math.PI ? "0":"1"; return `M ${r*Math.cos(sa)} ${r*Math.sin(sa)} A ${r} ${r} 0 ${large} 1 ${r*Math.cos(ea)} ${r*Math.sin(ea)}`; }; const path = `${arc(rOut,start,end)} L ${innerRadius*Math.cos(end)} ${innerRadius*Math.sin(end)} ${arc(innerRadius,end,start).replace('M','L').replace(new RegExp(`A ${innerRadius} ${innerRadius} 0 \\d 1`), `A ${innerRadius} ${innerRadius} 0 ${angle>Math.PI?1:0} 0`)} Z`; return (<path key={d.name} d={path} fill={colors[i%colors.length]} onMouseOver={()=>handleOver(i,d)} onMouseOut={handleOut} style={{transition:'all .18s', cursor:'pointer'}}/>); })}
                </g></svg>
            </div>
